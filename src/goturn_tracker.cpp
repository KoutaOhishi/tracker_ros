...
public:
  GoturnTracker(){
    ...
    this->sub_img = nh.subscribe(this->sub_img_name, 100, &GoturnTracker::sub_img_CB, this);
    this->sub_bbox = nh.subscribe(this->target_boundingBox_name, 10, &GoturnTracker::sub_bbox_CB, this);
    this->pub_bbox = nh.advertise<darknet_dnn::BoundingBox>("/tracker_ros/goturn/result_bbox", 1);
    this->pub_result = nh.advertise<std_msgs::Bool>("/tracker_ros/goturn/result_flag", 1);

    image_transport::ImageTransport it(this->nh);
    this->pub_img = it.advertise("/tracker_ros/goturn/result_img", 1);
  }
  ...
  void sub_img_CB(const sensor_msgs::ImageConstPtr& msg)
  {
    if(this->tracking_ctrl == true){
      ...
      if(bbox_init==false){
        ...
        cv::imshow("Tracker_ROS GOTURN Result", image);
...
int main(int argc, char** argv){
  ros::init(argc, argv, "goturn");
  ROS_INFO("### GOTURN_ROS Start ###\n");
